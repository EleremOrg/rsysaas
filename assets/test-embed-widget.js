async function sendGetRequestWithPayload() {
  ...
  try {
    ...
    await handleResponse(response, configWidget);
  } catch (error) {
...
async function handleResponse(response, configWidget) {
  const data = await response.json();
  if (response.ok) {
    await populateResults(data.data, configWidget);
  } else {
    console.error('Error inside the response:', data.message);
  };
}

async function populateResults(data, configWidget) {
  const widgetContainer = document.querySelector('.elerem-widget-container__widget');
  if (widgetContainer) {
    await addWidgetStyles();
    widgetContainer.innerHTML = await generateRecommendationHTML(data, configWidget);
  };
}

async function generateRecommendationHTML(data, configWidget) {
  let html = '';

  html += `<h2 class="elerem-recommendation-title">${configWidget.title}</h2>`;
