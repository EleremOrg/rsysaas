...
use super::models::{ProductCategory, ProductUpdateCategory};

pub async fn run_upsert_transaction(
    state: AppState,
    ...
    // let _ = sqlx::query("UPDATE products SET token = $1, scope = $2 WHERE pk = $3)")
...
pub async fn run_insert_transaction(
    state: AppState,
    payload: ProductCategory,
) -> Result<u64, AppError> {
    let mut tx = state
    ...
    let query_builder: QueryBuilder<Sqlite> =
        QueryBuilder::new("INSERT INTO products(id, company_pk, created_at, meta) ");
    let company_pk = 1;
